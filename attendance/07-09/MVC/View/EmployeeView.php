...
$valuesEmployee=unserialize(urldecode($_GET['value']));
// print_r($valuesEmployee);exit;
?>
<!DOCTYPE html> 
<head>
    <title>
      ...
      <button type="button" id="btn3" name="btn3" class="btn btn-default btn-sm" onclick="document.location='LoginView.php'">
        ...
        <?php                 
           foreach($valuesEmployee as $arr){
                   echo "<tr>";
                   echo "<td>".$arr['emp_id1']."</td>";
                   echo "<td>".$arr['emp_intime']."</td>";
                   echo "<td>".$arr['emp_outtime']."</td>";
                   echo "<td>".$arr['emp_shift']."</td>";
                   echo "</tr>";
                }
            ?>
            </tr>
